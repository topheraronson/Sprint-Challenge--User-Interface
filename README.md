...
1. If you were to describe semantic HTML to the next cohort of students, what would you say?
    Using descriptive tags to describe what the html is doing.

2. Name two big differences between ```display: block;``` and ```display: inline;```.
    ```display: block;``` Creates an new line and can use height and width in css while ```display: inline;``` is on a single line and ignores height and width in css.
...
5. Explain why git is valuable to a team of developers.
    It allows a group to work on the same file and merge their individual changes easily.
